addShortName <-
  function(data,
    ...
    if (is.null(shortNameRef)) {
      shortNameRef <- data #%>% #data should now always hold a shortName!
...
plotCohortComparisonStandardizedDifference <- function(balance,
  ...
  print("plotCohortComparisonStandardizedDifference: do this needs fix? todo?")
  balance <- balance %>%
    addShortName(
      shortNameRef = shortNameRef,
...
plotTemporalCompareStandardizedDifference <- function(balance,
  ...
  print("plotTemporalCompareStandardizedDifference: do this needs fix? todo?")
  balance <- balance %>%
    addShortName(
      shortNameRef = shortNameRef,
      cohortIdColumn = "cohortId1",
...
plotCohortOverlap <- function(data,
  ...
  print("plotCohortOverlap: do this needs fix? todo?")
  data <- data %>%
    addShortName(
      shortNameRef = shortNameRef,
