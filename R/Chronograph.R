...
executeChronographWrapper <- function(connectionDetails,
  ...
  for(i in 1:nrow(DEC_df)){
    ...
    if(create_pngs_in_extras_folder == TRUE){
      plotChronograph(data = chronograph_list[[i]], 
                      ...
                      DEC_i_cohort_ids[3], 
                      title = DEC_df[i,1], 
                      fileName = here::here(paste0("extras", "/" , DEC_df[i,1], ".jpeg")))
    }
  }
  ...
  data.table::fwrite(chronograph_data_all_decs, file=here::here(path_w_filename))
}
...
executeChronograph <- function(connectionDetails,
  ...
  # oracleTempSchema = NULL
  # cohortTable = "cohort_deci_oskarg"
  # cohortDatabaseSchema = cdmDatabaseSchema
  # targetCohortId = 22
  ...
  periodLength <- 6 # Number of days per bar
  numberOfPeriods <- 48 # Number of months we follow patients periodLength days * numberOfPeriods, e.g. 30 * 48 month = +/- 2 years 
  periodStarts <- c(
  ...
  sql <- SqlRender::readSql(here::here("inst/sql/sql_server/CreateChronographData.sql"))
  ...
  sql <- SqlRender::readSql(here::here("inst/sql/sql_server/ExtractChronographTable.sql"))
  sql <- SqlRender::translate(sql, targetDialect = connectionDetails$dbms)    
  sql <- SqlRender::render(sql,
                           cohort_database_schema = cohortDatabaseSchema)
  ...
  sql <- SqlRender::readSql(here::here("inst/sql/sql_server/DropChronographTables.sql"))
  sql <- SqlRender::translate(sql, targetDialect = connectionDetails$dbms)    
...
plotChronograph <- function(data=result_df, 
  ...
  plots <- list(topPanel, bottomPanel)
  grobs <- widths <- list()
