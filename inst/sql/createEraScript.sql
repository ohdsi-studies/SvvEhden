...
--	{DEFAULT @CONDITION_ERA_TABLE_NAME = 'condition_era' }  Not currently used
	{DEFAULT @DRUG_ERA_TABLE_NAME = 'drug_era_pw390' } 
	{DEFAULT @PERSISTENCE_WINDOW_IN_DAYS = '390' } 

--USE @TARGET_CDMV5;

IF OBJECT_ID('@TARGET_CDMV5_SCHEMA.@DRUG_ERA_TABLE_NAME', 'U') IS NOT NULL
    DROP TABLE @TARGET_CDMV5_SCHEMA.@DRUG_ERA_TABLE_NAME;

/****
DRUG ERA
Note: Eras derived from DRUG_EXPOSURE table, using @PERSISTENCE_WINDOW_IN_DAYS gap
 ****/
...
SELECT PERSON_ID
    ...
    ,DATEADD(day, - @PERSISTENCE_WINDOW_IN_DAYS, EVENT_DATE) AS END_DATE -- unpad the end date
INTO #cteEndDates
FROM (
    SELECT E1.PERSON_ID
    ...
    FROM (
        ...
        FROM (
            ...
            SELECT PERSON_ID
                ,INGREDIENT_CONCEPT_ID
                ,DATEADD(day, @PERSISTENCE_WINDOW_IN_DAYS, DRUG_EXPOSURE_END_DATE)
                ,1 AS EVENT_TYPE
                ,NULL
            FROM #cteDrugTarget
            ) RAWDATA
...
FROM (
    ...
    FROM (
        ...
        FROM (
            ...
            SELECT PERSON_ID
                ,CONDITION_CONCEPT_ID
                ,DATEADD(day, @PERSISTENCE_WINDOW_IN_DAYS, CONDITION_END_DATE)
                ,1 AS EVENT_TYPE
                ,NULL
            FROM #cteConditionTarget
            ) RAWDATA
