-- Creates the target drug cohorts to be used in SVVEHDEN-sprint, according to updated study protocol.
-- Note: cohort exit based on end of continuous drug exposure with death as censoring event
-- Note: based on template OHDSI.SQL generated from ATLAS 
--       see cohort-id 1778164 for example butalbital cohort (concept-id: 1139699)
-- TODO: add cohort with target drug and event to target database schema
/* 
##################################################################################
*/
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (@generic_concept_id)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (@generic_concept_id)
...
/* TO DO: MAY WANT TO MANUALLY ADJUST THE MAX AT RISK WINDOW HERE TO 2 YEARS FOR CHRONOGRAPH ANALYSES 
update #final_cohort
set end_date = dateadd(year,2,start_date)
where end_date > dateadd(year,2,start_date);
*/
