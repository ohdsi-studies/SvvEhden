  ...
  for (i in (1:nrow(cohorts))) {
    ...
    if (nrow(sql) == 0 || nrow(json) == 0) {
      ...
      if (!length(sql$conceptSetId %>% unique()) == length(json$conceptSetId %>% unique())) {
        ...
        print(cohort$sql)
        print(cohort$json)
        print(length(sql$conceptSetId %>% unique()))
        print(length(json$conceptSetId %>% unique()))
        print(sql$conceptSetId %>% unique())
        print(json$conceptSetId %>% unique())
...
instantiateUniqueConceptSets <- function(uniqueConceptSets,
  ...
  for (start in seq(1, length(sql), by = batchSize)) {
    ...
    if (stringr::str_detect(string = sqlSubset,
                            pattern = 'cdm_database_schema')) {
      sqlSubset <- SqlRender::render(sqlSubset,
...
runConceptSetDiagnostics <- function(connection,
  ...
  if (runIncludedSourceConcepts) {
    ...
    if (nrow(subsetIncluded) > 0) {
      ...
      if (useExternalConceptCountsTable) {
        ...
        packageName <- utils::packageName()
        if(is.null(packageName)) {packageName <- "CohortDiagnostics"}
  ...
  if (runBreakdownIndexEvents) {
    ...
    if (nrow(subsetBreakdown) > 0) {
      ...
      data <-
        lapply(split(subsetBreakdown, subsetBreakdown$cohortId),
               runBreakdownIndexEvents)
