...
loadCohortsFromCsvOutsidePackage <- function(cohortToCreateFile = "settings/cohortsToCreate.csv",
  ...
  cohorts <- readr::read_csv(pathToCsv,
                             col_types = readr::cols(),
                             guess_max = min(1e7))
  # If error: " Warning: vroom_parse_issue: One or more parsing issues, see `problems()` for details  "
  # readr::problems()
  ...
  getJson <- function(name, 
    ...
    json <- readChar(pathToJson, file.info(pathToJson)$size)
    json <- gsub(pattern = "||concept_id||",   replacement = concept_id,   x = json, fixed = TRUE)
    json <- gsub(pattern = "||concept_name||", replacement = concept_name, x = json, fixed = TRUE)
    json <- gsub(pattern = "||concept_code||", replacement = concept_code, x = json, fixed = TRUE)
    json <- gsub(pattern = "||fixed_TAR||", replacement = fixed_TAR, x = json, fixed = TRUE)
    
    if(items_only){
      # find "items" in json
      json_items <- tryCatch({  
        RJSONIO::toJSON(RJSONIO::fromJSON(json, digits = 23)$ConceptSets[[1]]$expression$items[[1]])
      }, error = function(e) {
        ""
      })
      return(json_items)
    ...
    #if intersection
    if(intersection_drug_cohort_id != 0 & intersection_event_cohort_id != 0) {
      json <- gsub(pattern = "||intersection_drug||", 
                   replacement = cohorts$json_items[cohorts$cohortId == intersection_drug_cohort_id], 
                   x = json, 
                   fixed = TRUE)
      json <- gsub(pattern = "||intersection_event||", 
                   replacement = cohorts$json_items[cohorts$cohortId == intersection_event_cohort_id], 
                   x = json, 
                   fixed = TRUE)
    }
    
    #debug part: if you want to view it manually
    #fileConn<-file(paste0("..\\inst\\cohorts\\last_",name,intersection_drug_cohort_id,"-",intersection_event_cohort_id,".json"))
    #write(json, fileConn)
    #close(fileConn)
  ...
  getSql <- function(name, concept_id, intersection_drug_cohort_id, intersection_event_cohort_id, fixed_TAR) { 
    ...
    sql <- readChar(pathToSql, file.info(pathToSql)$size)
    sql <- gsub(pattern = "@generic_concept_id",   replacement = concept_id,   x = sql, fixed = TRUE)
    sql <- gsub(pattern = "@all_drug_concept_ids",   replacement = paste(allDrugConceptIds, collapse =','),   x = sql, fixed = TRUE)
    sql <- gsub(pattern = "@intersection_drug_cohort_id",   replacement = intersection_drug_cohort_id,   x = sql, fixed = TRUE)
    sql <- gsub(pattern = "@intersection_event_cohort_id",   replacement = intersection_event_cohort_id,   x = sql, fixed = TRUE)
    sql <- gsub(pattern = "@fixed_TAR",   replacement = fixed_TAR,   x = sql, fixed = TRUE)
    
    if(intersection_drug_cohort_id != 0 & intersection_event_cohort_id != 0) {
      drug_concepts_json = cohorts$json_items[cohorts$cohortId == intersection_drug_cohort_id]
      event_concepts_json = cohorts$json_items[cohorts$cohortId == intersection_event_cohort_id]
      
      #Todo: this can be nicer done probably, by parsing the json
      concept_start <- gregexpr('"CONCEPT_ID"[ ]*:[ ]*', drug_concepts_json, fixed=FALSE) 
      concept_start <- concept_start[[1]] + attr(concept_start[[1]], "match.length") + 1
      intersectionDrugConceptIds <- vector()
      for(i in 1:length(concept_start)) {
        intersectionDrugConceptIds <- append(intersectionDrugConceptIds, 
                                             strsplit(substr(drug_concepts_json, 
                                                             concept_start[i], 
                                                             nchar(drug_concepts_json)), 
                                                      ",", 
                                                      fixed=TRUE)[[1]][1])
      }
      
      concept_start <- gregexpr('"CONCEPT_ID"[ ]*:[ ]*', event_concepts_json, fixed=FALSE) 
      concept_start <- concept_start[[1]] + attr(concept_start[[1]], "match.length") + 1
      intersectionEventConceptIds <- vector()
      for(i in 1:length(concept_start)) {
        intersectionEventConceptIds <- append(intersectionEventConceptIds, 
                                              strsplit(substr(event_concepts_json, 
                                                              concept_start[i], 
                                                              nchar(event_concepts_json)), 
                                                       ",", 
                                                       fixed=TRUE)[[1]][1])
      }
      
      sql <- gsub(pattern = "@generic_intersection_drug_concept_ids",   replacement = paste(intersectionDrugConceptIds %>% unique(), collapse =','),   x = sql, fixed = TRUE)
      sql <- gsub(pattern = "@generic_intersection_event_concept_ids",   replacement = paste(intersectionEventConceptIds %>% unique(), collapse =','),   x = sql, fixed = TRUE)
    }
    return(sql)
  }
...
selectColumnAccordingToResultsModel <- function(data) {
  ...
  columsToInclude <- c(columsToInclude, "concept_id", "concept_name", "concept_code", "matching_drug_cohort_id", "intersection_drug", "intersection_event")
  columsToInclude <- c(columsToInclude, "json", "sql")
  #columsToInclude <- c(columsToInclude, "sql")
