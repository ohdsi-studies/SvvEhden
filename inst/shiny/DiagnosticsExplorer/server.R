shiny::shinyServer(function(input, output, session) {
  ...
  # Function to return list of the 4 cohorts for a chosen DEC, in order
  ...
  cohortId <- shiny::reactive({
    if(verbose) print("cohortId")
    cohort_subset = cohortSubset()  #cohort exchanged with cohortSubset
    return(cohort_subset$cohortId[cohort_subset$compoundName == input$cohort])
  })
  
  cohortIds <- reactiveVal(NULL)
  shiny::observeEvent(eventExpr = {
    if(verbose) print("cohortIds")
    ...
    if (isFALSE(input$cohorts_open) || !is.null(input$tabs)) {
      cohort_subset = cohortSubset()  #cohort exchanged with cohortSubset
      selectedCohortIds <-
        cohort_subset$cohortId[cohort_subset$compoundName  %in% input$cohorts]
      cohortIds(selectedCohortIds)
    }
  })
  
  comparatorCohortId <- shiny::reactive({
    if(verbose) print("comparatorCohortId")
    cohort_subset = cohortSubset()  #cohort exchanged with cohortSubset
    return(cohort_subset$cohortId[cohort_subset$compoundName == input$comparatorCohort])
  })
  
  conceptSetIds <- shiny::reactive(x = {
    if(verbose) print("conceptSetIds")
  ...
  shiny::observeEvent(eventExpr = {
    if(verbose) print("timeIds <- reactiveVal(NULL")
  ...
  shiny::observeEvent(eventExpr = {
    if(verbose) print("databaseIds <- reactiveVal(NULL)")
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 111")
    subset <- cohortSubset()$compoundName
    shinyWidgets::updatePickerInput(
      session = session,
  ...
  getFormattedFileName <- function(fileName) {
    if(verbose) print("getFormattedFileName")
  ...
  cohortDefinitionTableData <- shiny::reactive(x = {
    if(verbose) print("cohortDefinitionTableData <- shiny::reactive")
    data <-  cohortSubset() %>%
  ...
  output$saveCohortDefinitionButton <- downloadHandler(
    filename = function() {
      if(verbose) print("output$saveCohortDefinitionButton <- downloadHandler")
  ...
  output$cohortDefinitionTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$cohortDefinitionTable <- DT::renderDataTable")
  ...
  selectedCohortDefinitionRow <- reactive({
    if(verbose) print("selectedCohortDefinitionRow")
  ...
  output$cohortDefinitionRowIsSelected <- reactive({
    if(verbose) print("output$cohortDefinitionRowIsSelected <- reactive")
  ...
  output$cohortDetailsText <- shiny::renderUI({
    if(verbose) print("output$cohortDetailsText <- shiny::renderUI")
  ...
  output$cohortCountsTableInCohortDefinition <- DT::renderDataTable(expr = {
    if(verbose) print("output$cohortCountsTableInCohortDefinition <- DT::renderDataTable")
    row <- selectedCohortDefinitionRow()
    if (is.null(row)) {
      return(NULL)
    } else {
  ...
  cohortDefinitionCirceRDetails <- shiny::reactive(x = {
    if(verbose) print("cohortDefinitionCirceRDetails")
  ...
  output$cohortDefinitionText <- shiny::renderUI(expr = {
    if(verbose) print("output$cohortDefinitionText <- shiny::renderUI")
    cohortDefinitionCirceRDetails()$htmlExpressionCohort %>%
      shiny::HTML()
  })
  
  output$cohortDefinitionJson <- shiny::renderText({
    if(verbose) print("output$cohortDefinitionJson <- shiny::renderText")
    row <- selectedCohortDefinitionRow()
    if (is.null(row)) {
      return(NULL)
  ...
  output$cohortDefinitionSql <- shiny::renderText({
    if(verbose) print("output$cohortDefinitionSql <- shiny::renderText")
    row <- selectedCohortDefinitionRow()
    if (is.null(row)) {
      return(NULL)
    } else {
  ...
  getConceptSetDataFrameFromConceptSetExpression <-
    function(conceptSetExpression) {
      if(verbose) print("getConceptSetDataFrameFromConceptSetExpression")
  ...
  getConceptSetDetailsFromCohortDefinition <-
    function(cohortDefinitionExpression) {
      if(verbose) print("getConceptSetDetailsFromCohortDefinition")
  ...
  cohortDefinistionConceptSetExpression <- shiny::reactive({
    if(verbose) print("cohortDefinistionConceptSetExpression")
    row <- selectedCohortDefinitionRow()
    if (is.null(row)) {
      return(NULL)
    }
  ...
  output$conceptsetExpressionTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$conceptsetExpressionTable <- DT::renderDataTable output$conceptsetExpressionTable")
  ...
  cohortDefinitionConceptSetExpressionRow <- shiny::reactive(x = {
    if(verbose) print("cohortDefinitionConceptSetExpressionRow")
  ...
  output$conceptSetExpressionRowSelected <- shiny::reactive(x = {
    if(verbose) print("output$conceptSetExpressionRowSelected <- shiny::reactive")
  ...
  output$isDataSourceEnvironment <- shiny::reactive(x = {
    if(verbose) print("output$isDataSourceEnvironment <- shiny::reactive")
  ...
  cohortDefinitionConceptSets <- shiny::reactive(x = {
    if(verbose) print("cohortDefinitionConceptSets")
  ...
  getDatabaseIdInCohortConceptSet <- shiny::reactive({
    if(verbose) print("getDatabaseIdInCohortConceptSet")
  ...
  getSubjectAndRecordCountForCohortConceptSet <- shiny::reactive(x = {
    if(verbose) print("getSubjectAndRecordCountForCohortConceptSet")
    row <- selectedCohortDefinitionRow()
  ...
  output$subjectCountInCohortConceptSet <- shiny::renderUI({
    if(verbose) print("output$subjectCountInCohortConceptSet <- shiny::renderUI")
  ...
  output$recordCountInCohortConceptSet <- shiny::renderUI({
    if(verbose) print("output$recordCountInCohortConceptSet <- shiny::renderUI")
    row <- getSubjectAndRecordCountForCohortConceptSet()
    if (is.null(row)) {
      return(NULL)
    } else {
  ...
  getResolvedOrMappedConceptSetForAllDatabase <-
    shiny::reactive(x = {
      if(verbose) print("getResolvedOrMappedConceptSetForAllDatabase")
      row <- selectedCohortDefinitionRow()
      if (is.null(row) ||
  ...
  getResolvedOrMappedConceptSetForAllVocabulary <-
    shiny::reactive(x = {
      if(verbose) print("getResolvedOrMappedConceptSetForAllVocabulary")
      data <- NULL
      row <- selectedCohortDefinitionRow()
      if (is.null(row) ||
          is.null(cohortDefinitionConceptSetExpressionRow()$id)) {
  ...
  getConceptCountForAllDatabase <- 
    shiny::reactive(x = {
      if(verbose) print("getConceptCountForAllDatabase")
      row <- selectedCohortDefinitionRow()
      if (is.null(row) ||
          is.null(cohortDefinitionConceptSetExpressionRow()$id)) {
        return(NULL)
  ...
  getResolvedOrMappedConcepts <- shiny::reactive({
    if(verbose) print("getResolvedOrMappedConcepts")
    data <- NULL
  ...
  output$saveCohortDefinitionIncludedResolvedConceptsTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveCohortDefinitionIncludedResolvedConceptsTable <-  downloadHandler")
  ...
  output$cohortDefinitionIncludedResolvedConceptsTable <-
    DT::renderDataTable(expr = {
      if(verbose) print("output$cohortDefinitionIncludedResolvedConceptsTable <- DT::renderDataTable")
  ...
  output$saveCohortDefinitionMappedConceptsTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveCohortDefinitionMappedConceptsTable <-  downloadHandler")
  ...
  output$cohortDefinitionMappedConceptsTable <-
    DT::renderDataTable(expr = {
      if(verbose) print("output$cohortDefinitionMappedConceptsTable <- DT::renderDataTable")
      data <- getResolvedOrMappedConcepts()
  ...
  output$saveCohortDefinitionConceptSetsTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveCohortDefinitionConceptSetsTable <-  downloadHandler")
  ...
  output$cohortDefinitionConceptSetsTable <-
    DT::renderDataTable(expr = {
      if(verbose) print("output$cohortDefinitionConceptSetsTable <- DT::renderDataTable")
  ...
  output$cohortConceptsetExpressionJson <- shiny::renderText({
    if(verbose) print("output$cohortConceptsetExpressionJson <- shiny::renderText")
    if (is.null(cohortDefinitionConceptSetExpressionRow())) {
      return(NULL)
    }
  ...
  output$saveConceptSetButton <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveConceptSetButton <- downloadHandler")
      getFormattedFileName(fileName = "ConceptSetsExpression")
    },
    content = function(file) {
      write.csv(cohortDefinitionConceptSets(), file)
  ...
  cohortDefinitionOrphanConceptTableData <- shiny::reactive(x = {
    if(verbose) print("cohortDefinitionOrphanConceptTableData")
  ...
  output$saveCohortDefinitionOrphanConceptsTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveCohortDefinitionOrphanConceptsTable <-downloadHandler")
  ...
  output$cohortDefinitionOrphanConceptTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$cohortDefinitionOrphanConceptTable <- DT::renderDataTable")
  ...
  getConceptSetIds <- shiny::reactive(x = {
    if(verbose) print("getConceptSetIds")
    return(conceptSets$conceptSetId[conceptSets$conceptSetName  %in% input$conceptSetsToFilterCharacterization])
  })
  
  getResoledAndMappedConceptIdsForFilters <- shiny::reactive({
    if(verbose) print("getResoledAndMappedConceptIdsForFilters")
  ...
  getCohortCountResultReactive <- shiny::reactive(x = {
    if(verbose) print("getCohortCountResultReactive start")
    ...
    data <- getCohortCountResult(
      ...
      addShortName(cohortSubset()) %>% #cohort exchanged with cohortSubset
      dplyr::arrange(.data$shortName, .data$databaseId)
    if(verbose) print("getCohortCountResultReactive end")
    return(data)
  })
  
  output$saveCohortCountsTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveCohortCountsTable <- downloadHandler")
  ...
  output$cohortCountsTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$cohortCountsTable <- DT::renderDataTable start")
    validate(need(length(databaseIds()) > 0, "No data sources chosen"))
    validate(need(length(cohortIds()) > 0, "No cohorts chosen"))
    ...
    if (nrow(data) == 0) {
      if(verbose) print("output$cohortCountsTable <- DT::renderDataTable end")
  ...
  getCohortIdOnCohortCountRowSelect <- reactive({
    if(verbose) print("getCohortIdOnCohortCountRowSelect start")
    idx <- input$cohortCountsTable_rows_selected
    if (is.null(idx)) {
      if(verbose) print("getCohortIdOnCohortCountRowSelect end")
      return(NULL)
    } else {
  ...
  output$cohortCountRowIsSelected <- reactive({
    if(verbose) print("output$cohortCountRowIsSelected <- reactive start")
    result <- !is.null(getCohortIdOnCohortCountRowSelect())
    if(verbose) print("output$cohortCountRowIsSelected <- reactive end")
    return(result)
  })
  ...
  output$InclusionRuleStatForCohortSeletedTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$InclusionRuleStatForCohortSeletedTable <- DT::renderDataTable")
    validate(need(length(databaseIds()) > 0, "No data sources chosen"))
    validate(need(
  ...
  incidenceRateData <- reactive({
    ...
    if(verbose) print("incidenceRateData <- reactive start")
    validate(need(length(databaseIds()) > 0, "No data sources chosen"))
    validate(need(length(cohortIds()) > 0, "No cohorts chosen"))
    ...
    if(verbose) print("incidenceRateData <- reactive end")
    if(verbose) if(verbose) print(data)
    return(data)
  })
  
  shiny::observe({
    if(verbose) print("shiny::observe 1503")
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 1525")
    if (!is.null(incidenceRateData()) &&
        nrow(incidenceRateData()) > 0) {
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 1547")
    if (!is.null(incidenceRateData()) &&
        nrow(incidenceRateData()) > 0) {
  ...
  shiny::observeEvent(eventExpr = {
    if(verbose) print("incidenceRateAgeFilter <- reactiveVal(NULL)")
  ...
  shiny::observeEvent(eventExpr = {
    if(verbose) print("incidenceRateGenderFilter <- reactiveVal(NULL)")
  ...
  incidenceRateCalenderFilter <- shiny::reactive({
    if(verbose) print("incidenceRateCalenderFilter start")
    calenderFilter <- incidenceRateData() %>%
      dplyr::select(.data$calendarYear) %>%
      dplyr::filter(.data$calendarYear != "NA",
                    !is.na(.data$calendarYear)) %>%
    ...
    if(verbose) print("incidenceRateCalenderFilter end")
    return(calenderFilter)
  })
  
  
  incidenceRateYScaleFilter <- shiny::reactive({
    if(verbose) print("incidenceRateYScaleFilter")
  ...
  output$saveIncidenceRatePlot <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveIncidenceRatePlot <-  downloadHandler")
  ...
  output$incidenceRatePlot <- ggiraph::renderggiraph(expr = {
    if(verbose) print("output$incidenceRatePlot <- ggiraph::renderggiraph start")
    validate(need(length(databaseIds()) > 0, "No data sources chosen"))
    validate(need(length(cohortIds()) > 0, "No cohorts chosen"))
    stratifyByAge <- "Age" %in% input$irStratification
    stratifyByGender <- "Sex" %in% input$irStratification
      ...
      message = paste(
        ...
        if (all(!is.null(data), nrow(data) > 0)) {
          plot <- plotIncidenceRate(
            data = data,
            shortNameRef = cohortSubset(), #cohort exchanged with cohortSubset,
            stratifyByAgeGroup = stratifyByAge,
            stratifyByGender = stratifyByGender,
            stratifyByCalendarYear = stratifyByCalendarYear,
            yscaleFixed = input$irYscaleFixed
          )
          if(verbose) print("output$incidenceRatePlot <- ggiraph::renderggiraph end")
          return(plot)
        }
  ...
  output$timeDisPlot <- ggiraph::renderggiraph(expr = {
    if(verbose) print("output$timeDisPlot <- ggiraph::renderggiraph")
    validate(need(length(databaseIds()) > 0, "No data sources chosen"))
    data <- timeDist()
    validate(need(nrow(data) > 0, paste0("No data for this combination")))
    plot <- plotTimeDistribution(data = data, shortNameRef = cohortSubset()) #cohort exchanged with cohortSubset)
    return(plot)
  })
  
  output$saveTimeDistTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveTimeDistTable <-  downloadHandler")
  ...
  output$timeDistTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$timeDistTable <- DT::renderDataTable")
    data <- timeDist()  %>%
      addShortName(cohortSubset()) %>% #cohort exchanged with cohortSubset
  ...
  includedConceptsData <- shiny::reactive(x = {
    if(verbose) print("includedConceptsData")
    validate(need(all(!is.null(databaseIds()), length(databaseIds()) > 0), 
                  "No data sources chosen"))
    validate(need(all(!is.null(cohortId()),length(cohortId()) > 0),
                  "No cohort chosen"))
  ...
  output$saveIncludedConceptsTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveIncludedConceptsTable <- downloadHandler")
  ...
  output$includedConceptsTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$includedConceptsTable <- DT::renderDataTable")
    validate(need(all(!is.null(databaseIds()), length(databaseIds()) > 0), 
                  "No data sources chosen"))
    validate(need(all(!is.null(cohortId()),length(cohortId()) > 0),
                  "No cohort chosen"))
  ...
  orphanConceptsData <- shiny::reactive(x = {
    if(verbose) print("orphanConceptsData")
  ...
  output$saveOrphanConceptsTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveOrphanConceptsTable <-  downloadHandler")
  ...
  output$orphanConceptsTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$orphanConceptsTable <- DT::renderDataTable")
    validate(need(length(databaseIds()) > 0, "No data sources chosen"))
  ...
  output$saveInclusionRuleTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveInclusionRuleTable <-  downloadHandler")
  ...
  output$inclusionRuleTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$inclusionRuleTable <- DT::renderDataTable")
    validate(need(length(databaseIds()) > 0, "No data sources chosen"))
    table <- getInclusionRuleStats(
      dataSource = dataSource,
      cohortIds = cohortId(),
  ...
  indexEventBreakDownData <- shiny::reactive(x = {
    if(verbose) print("indexEventBreakDownData")
  ...
  indexEventBreakDownDataFilteredByRadioButton <-
    shiny::reactive(x = {
      if(verbose) print("indexEventBreakDownDataFilteredByRadioButton")
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 2464")
    data <- domaintable()
    shinyWidgets::updatePickerInput(
      session = session,
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 2475")
  ...
  shiny::observeEvent(eventExpr = {
    if(verbose) print("selectedDomainTable <- reactiveVal(NULL)")
  ...
  shiny::observeEvent(eventExpr = {
    if(verbose) print("selectedDomainField <- reactiveVal(NULL)")
  ...
  output$saveBreakdownTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveBreakdownTable <-  downloadHandler")
  ...
  output$breakdownTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$breakdownTable <- DT::renderDataTable")
    validate(need(length(databaseIds()) > 0, "No data sources chosen"))
  ...
  output$saveVisitContextTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveVisitContextTable <-  downloadHandler")
  ...
  output$visitContextTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$visitContextTable <- DT::renderDataTable")
    validate(need(length(databaseIds()) > 0, "No data sources chosen"))
    validate(need(length(cohortId()) > 0, "No cohorts chosen"))
  ...
  getConceptSetNameForFilter <- shiny::reactive(x = {
    if(verbose) print("getConceptSetNameForFilter")
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 2962")
    subset <-
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 2975")
    subset <-
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 2988")
  ...
  output$saveCohortCharacterizationTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveCohortCharacterizationTable <-  downloadHandler")
  ...
  output$characterizationTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$characterizationTable <- DT::renderDataTable")
  ...
  output$saveTemporalCharacterizationTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveTemporalCharacterizationTable <-  downloadHandler")
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 3361")
    subset <-
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 3374")
    subset <-
  ...
  output$temporalCharacterizationTable <-
    DT::renderDataTable(expr = {
      if(verbose) print("output$temporalCharacterizationTable <- DT::renderDataTable")
  ...
  cohortOverlapData <- reactive({
    if(verbose) print("cohortOverlapData")
    validate(need(length(databaseIds()) > 0, "No data sources chosen"))
  ...
  output$overlapPlot <- ggiraph::renderggiraph(expr = {
    if(verbose) print("output$overlapPlot <- ggiraph::renderggiraph")
    validate(need(
      length(cohortIds()) > 0,
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 3569")
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 3581")
  ...
  output$saveCompareCohortCharacterizationTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveCompareCohortCharacterizationTable <-  downloadHandler")
  ...
  output$charCompareTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$charCompareTable <- DT::renderDataTable")
    ...
    targetCohortShortName <- cohortSubset() %>% #cohort exchanged with cohortSubset
      dplyr::filter(.data$cohortId == !!targetCohortIdValue) %>% 
      dplyr::select(.data$shortName) %>% 
      dplyr::pull()
    
    comparatorCohortShortName <- cohortSubset() %>% #cohort exchanged with cohortSubset
  ...
  output$charComparePlot <- ggiraph::renderggiraph(expr = {
    if(verbose) print("output$charComparePlot <- ggiraph::renderggiraph")
    data <- computeBalance()
  ...
  output$saveCompareTemporalCharacterizationTable <-  downloadHandler(
    filename = function() {
      if(verbose) print("output$saveCompareTemporalCharacterizationTable <-  downloadHandler")
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 3944")
    subset <-
  ...
  shiny::observe({
    if(verbose) print("shiny::observe 3957")
    subset <-
  ...
  output$temporalCharacterizationCompareTable <-
    DT::renderDataTable(expr = {
      if(verbose) print("output$temporalCharacterizationCompareTable <- DT::renderDataTable")
  ...
  output$temporalCharComparePlot <- ggiraph::renderggiraph(expr = {
    if(verbose) print("output$temporalCharComparePlot <- ggiraph::renderggiraph")
  ...
  output$databaseInformationTable <- DT::renderDataTable(expr = {
    if(verbose) print("output$databaseInformationTable <- DT::renderDataTable")
  ...
  targetCohortCount <- shiny::reactive({
    if(verbose) print("targetCohortCount")
    targetCohortWithCount <-
      ...
      dplyr::left_join(y = cohortSubset(), by = "cohortId") %>% #cohort exchanged with cohortSubset
      dplyr::arrange(.data$cohortName)
    return(targetCohortWithCount)
  })
  
  targetCohortCountHtml <- shiny::reactive({
    if(verbose) print("targetCohortCountHtml")
