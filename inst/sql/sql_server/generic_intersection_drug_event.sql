...
SELECT t.d_subject_id AS subject_id,
	   ...
	   CASE WHEN t.ev_cohort_start_date <= t.d_cohort_end_date -- set cohort end date to cohort exit date (from drug_cohort) or condition start date (from event_cohort) whichever occurs first
	        THEN t.ev_cohort_start_date     
	        ELSE t.d_cohort_end_date 
