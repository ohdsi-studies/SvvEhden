...
#' 
#' @export
generateCohortSetOutsidePackage <- function(connectionDetails = NULL,
                              connection = NULL,
                              cdmDatabaseSchema,
...
generateCohort <- function(cohortId = NULL,
  ...
  if (!incremental || isTaskRequired(cohortId = cohortDefinitionSet$cohortId[i],
    ...
    if (stringr::str_detect(string = sql, pattern = 'matching_drug_cohort_id')) {
      sql <- SqlRender::render(sql = sql,
                               matching_drug_cohort_id = cohortDefinitionSet$matching_drug_cohort_id[i],
    ...
    #fileConn<-file(paste0("..\\inst\\sql\\sql_server\\last_create_cohort_definition_script",cohortDefinitionSet$cohortId[i], ".sql"))
    #write(sql, fileConn)
    #close(fileConn)
