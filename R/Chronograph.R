...
#' @param cdmDatabaseSchema        Name of database schema that contains OMOP CDM and vocabulary. Note 
#'                                 that for SQL Server, this should include both the database and schema 
...
getChronographData <- function(connectionDetails,
  ...
  # cdmDatabaseSchema = "cdmOmop.synpuf5pct_20180710"
  # oracleTempSchema = NULL
  # cdmVersion = "5"
  # exposureIds = c()
  # outcomeIds = c(374040)
  # exposureOutcomePairs = structure(list(exposureId = 40170458L, outcomeId = 374040L), class = "data.frame", row.names = c(NA,
  # -1L))
  # exposureDatabaseSchema = cdmDatabaseSchema
  # exposureTable = "drug_era"
  # outcomeDatabaseSchema = cdmDatabaseSchema
  ...
  if(!exists("cdmDatabaseSchema")){
    stop("No provided cdmDatabaseSchema")
  }
  ...
  result$expectedCount <- as.numeric(result$observedCount) * result$allOutcomeCount/result$allObservedCount
  ...
  # Sort result-dataframe according to exposure, outcome and periodId
  result <- result[order(result$exposureId, result$outcomeId, result$periodId),]
